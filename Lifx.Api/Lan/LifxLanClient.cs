...
public partial class LifxLanClient(ILogger logger) : IDisposable
{
	...
	private async Task ReceiveLoopAsync(CancellationToken cancellationToken)
	{
		while (!cancellationToken.IsCancellationRequested && _socket is not null)
		{
			...
			{
				logger.LogDebug(ex, "Task cancelled");
			}
			catch (Exception ex)
			{
				logger.LogDebug(ex, "Error in receive loop: {Message}", ex.Message);
			}
		}
	}
	...
	private void HandleIncomingMessages(byte[] data, IPEndPoint endpoint)
	{
		...
		{
			if (taskCompletions.TryGetValue(msg.Source, out Action<LifxResponse>? tcs))
			{
				tcs(msg);
			}
			else
	...
	private Task<T?> BroadcastMessageAsync<T>(
		...
		params object[] args) where T : LifxResponse
	{
			...
			foreach (var arg in args)
			{
				switch (arg)
				{
					...
					case byte[]:
						...
						break;
					case string:
						payload.AddRange(Encoding.UTF8.GetBytes([.. ((string)arg).PadRight(32).Take(32)])); //All strings are 32 bytes
						break;
					default:
		...
		return BroadcastMessagePayloadAsync<T>(
			hostName,
			header,
			type,
			[.. payload],
	...
	private async Task<T?> BroadcastMessagePayloadAsync<T>(
		string? hostName,
		FrameHeader header,
		MessageType type,
		byte[] payload,
	...
	private static LifxResponse ParseMessage(byte[] packet)
	{
		...
		return LifxResponse.Create(header, type, source, size > 36 ? binaryReader.ReadBytes(size - 36) : []);
	}
	...
	private static void WritePacketToStream(Stream outStream, FrameHeader header, ushort type, byte[] payload)
	{
		...
		dw.Write([0, 0, 0, 0, 0, 0]); //reserved 1
